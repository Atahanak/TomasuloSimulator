...
class Program:
    ...
    def construct_stream(self, instruction_dictionary):
        ...
        for instruction in instruction_dictionary.values():
            ...
            destination = None if 'DEST' not in instruction.keys() else instruction['DEST']
            leap_address = None if 'ADDR' not in instruction.keys() else instruction['ADDR']
            stream[address] = Instruction(address, operation=operation, operand1=operand1, operand2 = operand2, destination=destination, leap_address=leap_address)
