...
class Instruction:
    def __init__(self, address, operation, operand1, operand2, destination, leap_address):
    ...
    def confirm_standard(self):
        ...
        if self.operation in BRANCH_OPERATIONS:
            if self.operand1 is None or self.operand2 is None or self.leap_address is None or self.destination is not None:
                raise 'INS: passed incorrect parameters to create a branch'
        elif self.operation in FP_OPERATIONS:
            if self.operand1 is None or self.operand2 is None or self.destination is None or self.leap_address is not None:
                raise 'INS: passed incorrect parameters to create an FP'
        elif self.operation in LOAD_OPERATIONS:
            if self.operand1 is None or self.destination is None or self.leap_address is not None or self.operand2 is not None:
